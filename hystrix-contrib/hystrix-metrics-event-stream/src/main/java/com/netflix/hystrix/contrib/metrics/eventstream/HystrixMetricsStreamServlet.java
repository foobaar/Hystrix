...
/**
 * Streams Hystrix metrics in text/event-stream format. <p> Install by: <p> 1) Including
 * hystrix-metrics-event-stream-*.jar in your classpath. <p> 2) Adding the following to web.xml:
 * <pre>{@code
 * <servlet>
...
public class HystrixMetricsStreamServlet extends HttpServlet {
  ...
  private static DynamicIntProperty maxConcurrentConnections = DynamicPropertyFactory.getInstance()
      .getIntProperty("hystrix.stream.maxConcurrentConnections", 5);
  ...
  /**
   * - maintain an open connection with the client - on initial connection send latest data of each requested event type
      ...
      if (numberConnections > maxConcurrentConnections.get()) {
        ...
        try {
          while (poller.isRunning() && !isDestroyed) {
            ...
            //check if the client has disconnected, if so throw an IOException
          ...
          logger
              .debug("IOException while trying to write (generally caused by client disconnecting). Will stop polling.",
                  e);
        } catch (Exception e) {
          poller.shutdown();
  ...
  private static class MetricJsonListener implements HystrixMetricsPoller.MetricsAsJsonPollerListener {
     ...
     * exception which causes the poller to stop. <p> This is a safety check against a runaway poller causing memory
     * leaks.
     */
